...
function AdminView({tpls,rules,bh,ann,closed,locs,fetchAll,tt}) {
  ...
  // ── Schedule Templates ──
  const[tplForm,setTplForm]=useState(null); // null=closed, {}=new, {template_id,...}=edit
  const[tplF,setTplF]=useState({template_name:"",start:"9:50a",end:"6:00p"});
  const[tplDel,setTplDel]=useState(null);

  const saveTpl=async()=>{
    if(!tplF.template_name.trim()){tt("Enter a template name");return;}
    const st=to24(tplF.start),et=to24(tplF.end);
    if(pT(tplF.end)<=pT(tplF.start)){tt("End must be after start");return;}
    try{
      if(tplForm?.template_id){
        await apiFetch(`/api/scheduling/templates/${tplForm.template_id}`,{method:"PUT",body:JSON.stringify({template_name:tplF.template_name,start_time:st,end_time:et})});
      } else {
        await apiFetch("/api/scheduling/templates",{method:"POST",body:JSON.stringify({template_name:tplF.template_name,start_time:st,end_time:et,location_id:null})});
      }
      await fetchAll();setTplForm(null);tt("Saved");
    }catch(e){tt(e.message);}
  };

  const delTpl=async id=>{
    try{await apiFetch(`/api/scheduling/templates/${id}`,{method:"DELETE"});await fetchAll();setTplDel(null);tt("Removed");}
    catch(e){tt(e.message);}
  };

  const openEditTpl=t=>{
    setTplF({template_name:t.template_name,start:apiTime(t.start_time),end:apiTime(t.end_time)});
    setTplForm(t);
  };

  const openNewTpl=()=>{
    setTplF({template_name:"",start:"9:50a",end:"6:00p"});
    setTplForm({});
  };
  ...
  const saveRule=async()=>{
    if(!ruleF.rule_name.trim()){tt("Enter a rule name");return;}
    try{
      if(ruleForm?.RULE_ID){
        await apiFetch(`/api/scheduling/admin/scheduling-rules/${ruleForm.RULE_ID}`,{method:"PUT",body:JSON.stringify(ruleF)});
      } else {
        await apiFetch("/api/scheduling/admin/scheduling-rules",{method:"POST",body:JSON.stringify(ruleF)});
      }
      await fetchAll();setRuleForm(null);tt("Saved");
    }catch(e){tt(e.message);}
  };

  const delRule=async id=>{
    try{await apiFetch(`/api/scheduling/admin/scheduling-rules/${id}`,{method:"DELETE"});await fetchAll();setRuleDel(null);tt("Removed");}
    catch(e){tt(e.message);}
  };

  const openEditRule=r=>{
    setRuleF({rule_name:r.RULE_NAME,rule_description:r.RULE_DESCRIPTION||"",rule_type:r.RULE_TYPE||"GENERAL",param_1:r.PARAM_1||"",param_2:r.PARAM_2||"",user_id:r.USER_ID||""});
    setRuleForm(r);
  };
  ...
  const saveAnn=async()=>{
    if(!annF.title.trim()||!annF.body.trim()){tt("Fill in title and message");return;}
    try{
      await apiFetch("/api/scheduling/admin/announcements",{method:"POST",body:JSON.stringify(annF)});
      await fetchAll();setAnnForm(false);setAnnF({title:"",body:""});tt("Posted");
    }catch(e){tt(e.message);}
  };

  const delAnn=async id=>{
    try{await apiFetch(`/api/scheduling/admin/announcements/${id}`,{method:"DELETE"});await fetchAll();setAnnDel(null);tt("Removed");}
    catch(e){tt(e.message);}
  };

  // ── Business Hours ──
  const[bhEdit,setBhEdit]=useState(null); // hours_id being edited
  const[bhF,setBhF]=useState({open_time:"",close_time:"",is_open:true});

  const openEditBh=h=>{
    setBhF({open_time:h.OPEN_TIME||"",close_time:h.CLOSE_TIME||"",is_open:h.IS_OPEN});
    setBhEdit(h.HOURS_ID);
  };

  const saveBh=async()=>{
    try{
      await apiFetch(`/api/scheduling/admin/business-hours/${bhEdit}`,{method:"PUT",body:JSON.stringify(bhF)});
      await fetchAll();setBhEdit(null);tt("Saved");
    }catch(e){tt(e.message);}
  };
  ...
  return <div style={{paddingTop:16}}>
      ...
      {tpls.map(t=><div key={t.template_id} style={CD}>
        {tplDel===t.template_id
          ...
          : <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
              <div>
                <div style={{fontWeight:600,fontSize:14,color:"#111"}}>{t.template_name}</div>
                <div style={{fontSize:13,color:"#6b7280",marginTop:2}}>{apiTime(t.start_time)} – {apiTime(t.end_time)} · {fmtH(t.hours_scheduled)}</div>
              </div>
              <div style={{display:"flex"}}>
                {editBtn(()=>openEditTpl(t))}
                {delBtn(()=>setTplDel(t.template_id))}
              </div>
            </div>}
      </div>)}
    ...
    {adminTab==="rules" && <div>
      ...
      {ruleForm!==null && formCard(<>
        {label("Rule Name")}
        <input style={{...SI,marginBottom:10}} placeholder="e.g. Max consecutive days" value={ruleF.rule_name} onChange={e=>setRuleF({...ruleF,rule_name:e.target.value})}/>
        {label("Description")}
        <input style={{...SI,marginBottom:10}} placeholder="Optional description" value={ruleF.rule_description} onChange={e=>setRuleF({...ruleF,rule_description:e.target.value})}/>
        <div style={{display:"flex",gap:10,marginBottom:10}}>
        ...
        {label("Param 2")}
        <input style={{...SI,marginBottom:12}} placeholder="Optional second parameter" value={ruleF.param_2} onChange={e=>setRuleF({...ruleF,param_2:e.target.value})}/>
        ...
        {ruleDel===r.RULE_ID
          ...
          : <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between"}}>
              ...
              <div style={{display:"flex",flexShrink:0}}>
                {editBtn(()=>openEditRule(r))}
                {delBtn(()=>setRuleDel(r.RULE_ID))}
              </div>
            </div>}
      </div>)}
    ...
    {adminTab==="announcements" && <div>
      ...
      {annForm && formCard(<>
        {label("Title")}
        <input style={{...SI,marginBottom:10}} placeholder="Announcement title" value={annF.title} onChange={e=>setAnnF({...annF,title:e.target.value})}/>
        {label("Message")}
        <textarea style={{...SI,marginBottom:12,resize:"vertical",minHeight:80}} placeholder="Message body..." value={annF.body} onChange={e=>setAnnF({...annF,body:e.target.value})}/>
        ...
        {annDel===a.announcement_id
          ...
          : <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between"}}>
              <div style={{flex:1}}>
                ...
                <div style={{fontSize:13,color:"#6b7280",marginTop:4}}>{a.body}</div>
              </div>
            ...
            {bhEdit===h.HOURS_ID
              ? <div style={{background:"#f0f7ff",borderRadius:8,padding:12,marginBottom:8}}>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
                    <label style={{display:"flex",alignItems:"center",gap:6,fontSize:13,cursor:"pointer"}}>
                      <input type="checkbox" checked={bhF.is_open} onChange={e=>setBhF({...bhF,is_open:e.target.checked})}/> Open
                    </label>
                  </div>
              ...
              : <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 0",borderBottom:"1px solid #f3f4f6",fontSize:13}}>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    <span style={{color:"#6b7280"}}>{h.IS_OPEN?`${h.OPEN_TIME} – ${h.CLOSE_TIME}`:"Closed"}</span>
                    {editBtn(()=>openEditBh(h))}
                  </div>
                </div>}
          </div>
        )}
...
function App() {
  ...
  return <div style={{background:"#f8f9fb",minHeight:"100vh",maxWidth:960,margin:"0 auto"}}>
    ...
    <div style={{padding:"0 16px 100px"}}>
      ...
      {view==="admin"&&canE&&<AdminView {...{tpls,rules,bh,ann,closed,locs,fetchAll,tt}}/>}
    </div>
